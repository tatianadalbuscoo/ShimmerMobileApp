...
namespace ShimmerSDK.IMU
{
    ...
    public partial class ShimmerSDK_IMU
    {
        ...
        /// <summary>
        /// Formats a value as a short, culture-invariant string (3 decimals).
        /// </summary>
        /// <param name="v">
        /// The value to format. Supports primitive numeric types, <see cref="NumericPayload"/>,
        /// or any object exposing a numeric <c>Data</c> property.
        /// </param>
        /// <returns>
        /// A string with 3 fractional digits for numeric values; otherwise the result of
        /// <c>ToString()</c>, or <c>"-"</c> when <paramref name="v"/> (or its <c>Data</c>) is <c>null</c>.
        /// </returns>
        /// <remarks>
        /// Formatting uses <see cref="CultureInfo.InvariantCulture"/> to ensure a dot as decimal
        ...
        /// <summary>
        /// Opens (or reuses) the WebSocket to the Android bridge, performs the initial
        /// handshake, subscribes to the target MAC, and pushes the current sensor config.
        /// </summary>
        /// <returns>A task that completes once the handshake, subscription and config push have been attempted.</returns>
        /// <exception cref="InvalidOperationException">
        ...
        private async Task ConnectMacAsync()
        {
            ...
            // Robust subscription to the target MAC 
            _subscribed = false;
            await EnsureSubscribedAsync().ConfigureAwait(false);

            // Soft configuration push: best-effort 
        ...
        /// <summary>
        /// Requests the firmware to apply the sampling rate closest to <paramref name="desiredHz"/> via the bridge,
        /// waits for the acknowledgement, and updates <c>SamplingRate</c> to the value actually applied.
        /// </summary>
        /// <param name="desiredHz">Desired sampling rate in Hertz. Must be greater than zero.</param>
        /// <returns>The sampling rate (in Hertz) that the firmware actually applied (nearest available).</returns>
        /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="desiredHz"/> is less than or equal to zero.</exception>
        /// <exception cref="InvalidOperationException">
        /// Thrown when <c>BridgeTargetMac</c> is not set, or when the bridge reports failure applying the rate.
        /// </exception>
        /// <exception cref="TimeoutException">Thrown if the bridge does not acknowledge the rate change within the timeout.</exception>
        public async Task<double> SetFirmwareSamplingRateNearestAsync(double desiredHz)
        {
            ...
            // Update local model and return the applied rate.
            ...
            try
            {
                ...
                switch (type)
                {
                    ...
                    case "error":
                        ...
                        // Signal failure to pending waiters (open/config/start).
